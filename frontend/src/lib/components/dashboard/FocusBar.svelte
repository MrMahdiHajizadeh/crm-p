<script>
  ...
  const items = $derived([
    { href: '/tasks?filter=overdue', icon: AlertCircle, iconClass: 'text-[var(--task-overdue)]', bgClass: 'bg-[var(--task-overdue-bg)] dark:bg-[var(--task-overdue)]/15', count: overdueCount, labelKey: 'focusbar.overdue', show: overdueCount > 0 },
    { href: '/tasks?filter=today', icon: Calendar, iconClass: 'text-[var(--task-due-today)]', bgClass: 'bg-[var(--task-due-today-bg)] dark:bg-[var(--task-due-today)]/15', count: todayCount, labelKey: 'focusbar.due_today', show: true },
    { href: '/leads?filter=followup_today', icon: Phone, iconClass: 'text-[var(--activity-call)]', bgClass: 'bg-[var(--color-success-light)] dark:bg-[var(--activity-call)]/15', count: followupsCount, labelKey: 'focusbar.followups', show: true },
    { href: '/leads?rating=HOT', icon: Flame, iconClass: 'text-[var(--lead-hot)]', bgClass: 'bg-[var(--lead-hot-bg)] dark:bg-[var(--lead-hot)]/15', count: hotLeadsCount, labelKey: 'focusbar.hot_leads', show: hotLeadsCount > 0 }
  ]);
    ...
    <div class="relative flex items-center gap-3 sm:gap-5">
      ...
      <div class="flex flex-1 items-center gap-1 overflow-x-auto sm:gap-2">
        {#each visibleItems as item}
          <a href={item.href} class="group flex items-center gap-2 rounded-[var(--radius-md)] px-3 py-2 transition-all duration-200 hover:bg-white/50 sm:gap-2.5 sm:px-4 dark:hover:bg-white/5">
            ...
            <div class="flex items-center gap-2">
              <span class="text-base font-bold text-[var(--text-primary)] tabular-nums">{item.count}</span>
              <span class="hidden text-xs font-medium text-[var(--text-secondary)] sm:inline">{$_(item.labelKey)}</span>
            </div>
          </a>
        {/each}
      </div>
