  ...
  : onRowClick ? 'hover:bg-[color:var(--bg-hover)]' : ''}"
          ...
          >
            ...
            {#each visibleColumnDefs as column, colIndex (column.key)}
              ...
              <td
                class="h-[37px] px-[14px] text-[12px] {colIndex === 0 ? 'ps-7 md:ps-8 font-medium text-[color:var(--text)]' : 'text-[color:var(--text-muted)]'} {colIndex === 0 && activeRowId === row[rowIdKey] ? 'shadow-[inset_2.5px_0_0_var(--text)]' : ''} {column.width || ''}"
              >
                {#if cellContent}
