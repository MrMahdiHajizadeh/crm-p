<script>
  ...
  $effect(() => {
    if (form?.success) {
      ...
      // Update the reactive store so the sidebar picks up feature flag changes
      // immediately without requiring a JWT refresh or full page reload.
      if (form.settings) {
        initOrgSettings(form.settings);
      }
    } else if (form?.error) {
